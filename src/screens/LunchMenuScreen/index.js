...
class LunchMenuScreen extends Component {
  ...
  renderLunch = () => {
    const toRender = [];
    const options = ['Kött', 'Fisk', 'Veggo'];
    // Replace lunchMenu with this.props.lunchMenu
    for (let day in lunchMenu) {
      let i = -1;
      toRender.push(
        <Card style={{ padding: '3%', width: '98%', ...commonStyles.card }} key={day}>
          ...
          {lunchMenu[day].map(dish => {
            ...
            return (
              <Text style={lunchMenuStyles.dishText} key={dish}>
                <Text style={{fontWeight: 'bold'}}>{options[i]}</Text> {dish}
              </Text>
            );
          })}
        </Card>
  ...
  render() {
    return (
      ...
      <NavigationBar styleName="inline" title={`Lunch meny v. ${new Date().getWeek()}`} />
      <ScrollView
